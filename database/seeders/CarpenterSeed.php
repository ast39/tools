...
class CarpenterSeed extends Seeder
{
    ...
    public function run(): void
    {
        $master = Sphere::create([
            'title' => 'Плотник',
            'body' => '<p>Во время проведения ремонта в любом помещении большой объем работы приходится на плотника и
            столяра. Ни в одном интерьере нельзя обойтись без конструкций и изделий из дерева. Тем более, в последнее
            время этот материал стал очень популярным у дизайнеров.</p>',
        ])->id;

        $cat = Category::create([
            'sphere_id' => $master,
            'title' => 'Плотницкие работы',
            'body' => '<p>Плотницкие услуги не предполагают чистовой отделки деревянных поверхностей. Мастерство хорошего
            плотника заключается в умении правильно произвести рубку, пиление, строгание и др. Кроме того, сделать
            верный подбор дерева, продумать способ соединения различных элементов деревянной конструкции.</p>'
        ])->id;
